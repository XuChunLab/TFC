...
filepath= 'D:\miniscope analysis\bt_miniscope\dca1\sigraw\combine2\method5\ext1';
% filenames=dir([ filepath, '\*.mat']);
...
filepath= 'D:\miniscope analysis\bt_miniscope\dca1\sigraw\combine2\method5\ext2';
% filenames=dir([ filepath, '\*.mat']);
...
stable_activated=intersect(habituation.all_tone_final,fearTest.all_tone_final); % control 第一类cell，responsive stable
from_0_1=setdiff(fearTest.all_tone_final,stable_activated); %从无到有反应，无的状态里包含了-1和0 第二类
% 第1类 fear neuron
temp1=intersect(fearTest.all_tone_final,laterExtinct.all_tone_final);
temp2=setdiff(fearTest.all_tone_final,temp1);
fear_neuron=intersect(from_0_1,temp2);
% common group
s1_s2_overlap=intersect(habituation.all_tone_final,fearTest.all_tone_final);
s1_s3_overlap=intersect(habituation.all_tone_final,laterExtinct.all_tone_final);
s2_s3_overlap=intersect(fearTest.all_tone_final,laterExtinct.all_tone_final);
% type 2 fear inhibited neuron
tmp1=setdiff(habituation.all_tone_final,s1_s2_overlap);
fear_inh=intersect(tmp1,laterExtinct.all_tone_final);
% type3 persistent
persistent_cell=intersect(s1_s2_overlap,laterExtinct.all_tone_final); % 此类即为persistent cell
% type 4 ext resist
tmp2=setdiff(fearTest.all_tone_final,s1_s2_overlap);
ext_resist=intersect(tmp2,laterExtinct.all_tone_final);
% type 5 ext up
tmp3=setdiff(laterExtinct.all_tone_final,s2_s3_overlap);
tmp4=intersect(tmp3,habituation.all_tone_final);
ext_up=setdiff(tmp3,tmp4);
% type 6 novel
tmp5=setdiff(habituation.all_tone_final,s1_s2_overlap);
tmp6=intersect(tmp5,laterExtinct.all_tone_final);
novel=setdiff(tmp5,tmp6);
%% 指定neuron ID
number=1;
neuron_id=fear_neuron;
...
% new colormap
% [c1,~]=colorGradient(RGB('red'),RGB('darkred'),64*2); % red to black
% [c3,~]=colorGradient(RGB('white'), RGB('red'),64*2); % blue to yellow
% [c4,~]=colorGradient(RGB('blue'),RGB('white'),64*2);
% [c5,~]=colorGradient(RGB('midnightblue'),RGB('blue'),64*2);
% c = [c5;c4;c3;c1];
...
hold on;
%ylim([-2,3]);
plot([100,100],[-0.5,1.5],'-k','linewidth',2);
%% 分开画
figure('Position',[100, 400, 300,150]);
% habituation phase;
%SR_id_increase= ;
...
xticks([]); 
hold on; xlim([0,1500]); ylim([-2,5]);
SetFigure;axis off;
xline( 300, '--','color',[0.18 0.3 0.3],'linewidth',2); xline(1200,'--','color',[0.18 0.3 0.3],'linewidth',2);
hold on;
...
SetFigure;axis off;
hold on;
xline( 300, '--','color',[0.18 0.3 0.3],'linewidth',2); xline(1200,'--','color',[0.18 0.3 0.3],'linewidth',2);%xline(xline3,'-r','linewidth',2); xticks([]);
hold on;ylim([-2,5]);
figure('Position',[100, 400, 300,150]);
% ext phase
response_activity=whole_responsezmean3(SR_id_increase,300:1800);
