...
clc;clear;
filepath= 'D:\miniscope analysis\bt_miniscope\dca1\sigraw\combine2\method5\habituation5'; % habituation5 data is in the data resource folder
...
trace_specific=setdiff(habituation.all_trace_final,tone_trace);
...
%% plot tone-only cell heat map
xline1 = 20*30; % tone ON
xline2 = 20*30 + 30 * 30; % 20 sec * 30 Hz % trace ON
xline3 = 20*30 + 50*30+40*30; % 40 sec * 30Hz % trace OFF
% plot raw heat map
...
figure('Position',[300, 100, 350,288]);
...
imagesc(whole_responsezmean(plot_tone_id,:),[1.65,6]);%colorbar % [1.65,6]
hold on;
colorbar;
...
c = [c5;c4;c3;c2;c1];
clear valmax;clear indmax;clear indsort;
figure('Position',[300, 100, 320,288]);
...
%% 定制的colormap来画图 for trace-only
% customize colormap
xline1 = 20*30; 
xline2 = 20*30 + 30 * 30;
xline3 = 20*30 + 50*30+40*30; 
...
c = [c5;c4;c3;c2;c1];
clear valmax;clear indmax;clear indsort;
figure('Position',[300, 100, 320,386]);
...
c = [c5;c4;c3;c2;c1];
clear valmax;clear indmax;clear indsort;
figure('Position',[300, 100, 320,192]);
[valmax,indmax]=max(tone_trace_resp(habi_persistent,:),[],2);% indmax is represent the frame ID for max response.
% sort the cell order by the earlist frame ID with max response.
[~,indsort]=sort(indmax); 
plot_tone_id=habi_persistent(indsort);
imagesc(whole_responsezmean(plot_tone_id,:));
colormap(c); 
caxis([-10 10]);
%colorbar;
...
%% tone-trace cell
clear valmax;clear indmax;clear indsort;
figure('Position',[300, 100, 350,192]);
...
%%
clear valmax;clear indmax;clear indsort;
figure('Position',[300, 100, 350,354]);
[valmax,indmax]=max(trace_responsezmean(trace_only,:),[],2);
% indmax is represent the frame ID for max response.
% sort the cell order by the earlist frame ID with max response.
[~,indsort]=sort(indmax); 
plot_trace_id=trace_only(indsort);
imagesc(whole_responsezmean(plot_trace_id,:),[1.65,6]);colorbar
hold on;
%area([xline3; xline3+shock_dur],[ylim; ylim],'FaceAlpha',0.4,'FaceColor','r','LineStyle','none')
xline( xline1, '-r','linewidth',1); xline(xline2,'-r','linewidth',1);xline(xline3,'-r','linewidth',1); xticks([]); 
